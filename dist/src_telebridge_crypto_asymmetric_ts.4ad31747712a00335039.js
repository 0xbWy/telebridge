"use strict";
(self["webpackChunktelebridge"] = self["webpackChunktelebridge"] || []).push([["src_telebridge_crypto_asymmetric_ts"],{

/***/ "./src/telebridge/crypto/asymmetric.ts"
/*!*********************************************!*\
  !*** ./src/telebridge/crypto/asymmetric.ts ***!
  \*********************************************/
...
/* harmony export */   decryptAsymmetricRecipient: () => (/* binding */ decryptAsymmetricRecipient),
/* harmony export */   decryptAsymmetricSelf: () => (/* binding */ decryptAsymmetricSelf),
/* harmony export */   decryptSecuredMessageRecipient: () => (/* binding */ decryptSecuredMessageRecipient),
/* harmony export */   decryptSecuredMessageSelf: () => (/* binding */ decryptSecuredMessageSelf),
/* harmony export */   encryptAsymmetric: () => (/* binding */ encryptAsymmetric),
/* harmony export */   encryptSecuredMessage: () => (/* binding */ encryptSecuredMessage)
/* harmony export */ });
/* harmony import */ var _noble_curves_ed25519_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @noble/curves/ed25519.js */ "./node_modules/@noble/curves/ed25519.js");
/* harmony import */ var _noble_hashes_hkdf_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @noble/hashes/hkdf.js */ "./node_modules/@noble/hashes/hkdf.js");
/* harmony import */ var _noble_hashes_sha2_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @noble/hashes/sha2.js */ "./node_modules/@noble/hashes/sha2.js");
/* harmony import */ var _identity__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./identity */ "./src/telebridge/crypto/identity.ts");
/* harmony import */ var _symmetric__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./symmetric */ "./src/telebridge/crypto/symmetric.ts");
/**
 ...
 */
function deriveMessageKeyFromECDH(dhOutput, info = SECURED_MESSAGE_INFO) {
  return new Uint8Array((0,_noble_hashes_hkdf_js__WEBPACK_IMPORTED_MODULE_1__.hkdf)(_noble_hashes_sha2_js__WEBPACK_IMPORTED_MODULE_2__.sha256, dhOutput, new Uint8Array(KEY_LENGTH), info, KEY_LENGTH));
}
 ...
 */
async function encryptAsymmetric(plaintext, recipientX25519Pub, senderX25519Pair, senderEd25519Priv) {
  // Validate inputs
  ...
  const ephPair = _noble_curves_ed25519_js__WEBPACK_IMPORTED_MODULE_0__.x25519.keygen();
  const ephemeralPriv = ephPair.secretKey;
  const ephPub = ephPair.publicKey;
  try {
    // Sign the plaintext with the sender's Ed25519 identity
    const signature = (0,_identity__WEBPACK_IMPORTED_MODULE_3__.signBytes)(senderEd25519Priv, plaintext);

    // ---- Encrypt for recipient ----
    // DH output: ECDH(ephemeralPriv, recipientX25519Pub)
    const recipientDhOutput = _noble_curves_ed25519_js__WEBPACK_IMPORTED_MODULE_0__.x25519.getSharedSecret(ephemeralPriv, recipientX25519Pub);
    ...
    } = await (0,_symmetric__WEBPACK_IMPORTED_MODULE_4__.encryptSymmetric)(plaintext, recipientDerivedKey);

    // Build recipient payload: [ephPub (32B)][nonce (12B)][ciphertext (var)][authTag (16B)][signature (64B)]
    const forRecipient = concat(ephPub,
    // 32B
    recipientNonce,
    // 12B
    recipientCiphertext,
    ...
    const selfDhOutput = _noble_curves_ed25519_js__WEBPACK_IMPORTED_MODULE_0__.x25519.getSharedSecret(ephemeralPriv, senderX25519Pair.point);
    const selfDerivedKey = deriveMessageKeyFromECDH(selfDhOutput, SECURED_SELF_INFO);
    const {
      nonce: selfNonce,
      ciphertext: selfCiphertext,
      authTag: selfAuthTag
    } = await (0,_symmetric__WEBPACK_IMPORTED_MODULE_4__.encryptSymmetric)(plaintext, selfDerivedKey);

    // Build self-copy payload: [ephPub (32B)][nonce (12B)][ciphertext (var)][authTag (16B)][signature (64B)]
    const forSelf = concat(ephPub,
    // 32B
    selfNonce,
    // 12B
    selfCiphertext,
 ...
 */
async function decryptAsymmetricRecipient(payload, recipientX25519Priv, senderEd25519Pub) {
  ...
  const dhOutput = _noble_curves_ed25519_js__WEBPACK_IMPORTED_MODULE_0__.x25519.getSharedSecret(recipientX25519Priv, ephemeralPubKey);
  ...
  const plaintext = await (0,_symmetric__WEBPACK_IMPORTED_MODULE_4__.decryptSymmetric)(nonce, ciphertext, authTag, derivedKey);

  // Verify Ed25519 signature
  const isSignatureValid = (0,_identity__WEBPACK_IMPORTED_MODULE_3__.verifySignature)(senderEd25519Pub, signature, plaintext);
  return {
    plaintext,
    senderEd25519Pub,
    isSignatureValid
 ...
 * @throws Error on invalid payload format, decryption failure, or auth tag mismatch
 */
async function decryptAsymmetricSelf(payload, senderX25519Priv, senderEd25519Pub) {
  if (!isUint8Array(payload) || payload.length < MIN_PAYLOAD_SIZE) {
    throw new Error(`Payload must be at least ${MIN_PAYLOAD_SIZE} bytes, got ${payload.length}`);
  }
  ...
  const dhOutput = _noble_curves_ed25519_js__WEBPACK_IMPORTED_MODULE_0__.x25519.getSharedSecret(senderX25519Priv, ephemeralPubKey);
 ...
 */
async function encryptSecuredMessage(plaintext, recipientX25519Pub, senderIdentityKeypair) {
  const senderX25519 = (0,_identity__WEBPACK_IMPORTED_MODULE_3__.deriveX25519FromEd25519)(senderIdentityKeypair.signingBytes);
  return encryptAsymmetric(plaintext, recipientX25519Pub, senderX25519, senderIdentityKeypair.signingBytes);
}
 ...
 * @returns Decrypted plaintext and signature verification result
 */
async function decryptSecuredMessageRecipient(payload, recipientIdentityKeypair, senderEd25519Pub) {
  const recipientX25519 = (0,_identity__WEBPACK_IMPORTED_MODULE_3__.deriveX25519FromEd25519)(recipientIdentityKeypair.signingBytes);
  return decryptAsymmetricRecipient(payload, recipientX25519.scalar, senderEd25519Pub);
}
 ...
 * @returns Decrypted plaintext and signature verification result
 */
async function decryptSecuredMessageSelf(payload, senderIdentityKeypair) {
  const senderX25519 = (0,_identity__WEBPACK_IMPORTED_MODULE_3__.deriveX25519FromEd25519)(senderIdentityKeypair.signingBytes);
  return decryptAsymmetricSelf(payload, senderX25519.scalar, senderIdentityKeypair.verifyingBytes);
}

/***/ }

}]);
//# sourceMappingURL=src_telebridge_crypto_asymmetric_ts.4ad31747712a00335039.js.map