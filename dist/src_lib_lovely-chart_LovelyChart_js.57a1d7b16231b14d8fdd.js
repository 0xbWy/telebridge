"use strict";
(self["webpackChunktelebridge"] = self["webpackChunktelebridge"] || []).push([["src_lib_lovely-chart_LovelyChart_js"],{
...
function createAxes(context, data, plotSize, colors) {
  ...
  function drawYAxis(state, projection, secondaryProjection) {
    ...
    if (data.isPercentage) {
      ...
      _drawYAxisScaled(state, projection, Math.round(yAxisScaleTo || yAxisScale), yMinViewportTo !== undefined ? yMinViewportTo : yMinViewport, yMaxViewportTo !== undefined ? yMaxViewportTo : yMaxViewport, yAxisScaleFrom ? yAxisScaleProgress : 1);
      _drawSecondaryYAxis(state, projection, Math.round(yAxisScaleTo || yAxisScale), yMinViewportTo !== undefined ? yMinViewportTo : yMinViewport, yMaxViewportTo !== undefined ? yMaxViewportTo : yMaxViewport, yAxisScaleFrom ? yAxisScaleProgress : 1, data.secondaryYAxis);
    } else {
  ...
  function _drawYAxisScaled(state, projection, scaleLevel, yMin, yMax, opacity = 1, colorKey = null, isSecondary = false) {
    ...
    for (let value = firstVisibleValue; value <= lastVisibleValue; value += step) {
      ...
      const label = isSecondary ? (0,_format_js__WEBPACK_IMPORTED_MODULE_1__.humanize)(value) : `${data.valuePrefix || ''}${(0,_format_js__WEBPACK_IMPORTED_MODULE_1__.humanize)(value)}${data.valueSuffix || ''}`;
      if (!isSecondary) {
        context.fillText(label, _constants_js__WEBPACK_IMPORTED_MODULE_0__.GUTTER, yPx - _constants_js__WEBPACK_IMPORTED_MODULE_0__.GUTTER / 2);
      } else {
        context.fillText(label, plotSize.width - _constants_js__WEBPACK_IMPORTED_MODULE_0__.GUTTER, yPx - _constants_js__WEBPACK_IMPORTED_MODULE_0__.GUTTER / 2);
      }
      if (isSecondary) {
  ...
  function _drawSecondaryYAxis(state, projection, scaleLevel, yMin, yMax, opacity = 1, secondaryYAxis) {
    ...
    for (let value = firstVisibleValue; value <= lastVisibleValue; value += step) {
      ...
      context.fillStyle = (0,_skin_js__WEBPACK_IMPORTED_MODULE_2__.getCssColor)(colors, 'y-axis-text', textOpacity);
      context.fillText(`${prefix}${(0,_format_js__WEBPACK_IMPORTED_MODULE_1__.humanize)(secondaryValue)}${suffix}`, plotSize.width - _constants_js__WEBPACK_IMPORTED_MODULE_0__.GUTTER, yPx - _constants_js__WEBPACK_IMPORTED_MODULE_0__.GUTTER / 2);
    }
  }
  return {
    drawXAxis,
...
function createTools(container, data, filterCallback) {
  ...
  function _setupLayout() {
    ...
    data.datasets.forEach(({
      ...
      const darkContent = (0,_skin_js__WEBPACK_IMPORTED_MODULE_2__.isColorCloseToWhite)(data.colors[key]) ? ' lovely-chart--dark-content' : '';
...
function createTooltip(container, data, plotSize, colors, onZoom, onFocus) {
  ...
  function _insertNewDataSet(dataSetContainer, {
    ...
    const colorClass = (0,_skin_js__WEBPACK_IMPORTED_MODULE_4__.isColorCloseToBackground)(colors, colorHex) ? '' : ` lovely-chart--color-${colorHex.slice(1)}`;
  ...
  function _formatValue(value) {
    return `${data.valuePrefix || ''}${(0,_format_js__WEBPACK_IMPORTED_MODULE_3__.formatInteger)(value)}${data.valueSuffix || ''}`;
  }
  ...
  function _updateDataSets(statistics) {
    ...
    const limitedStatistics = sortedStatistics.slice(0, _constants_js__WEBPACK_IMPORTED_MODULE_1__.MAX_TOOLTIP_ITEMS);
    const finalStatistics = data.isPie ? limitedStatistics.filter(({
      value
    }, index) => _isPieSectorSelected(statistics, value, totalValue, index, pointerVector)) : limitedStatistics;
...
/* harmony export */   MAX_TOOLTIP_ITEMS: () => (/* binding */ MAX_TOOLTIP_ITEMS),
...
function drawDatasets(context, state, data, range, points, projection, secondaryPoints, secondaryProjection, lineWidth, visibilities, colors, pieToBar, simplification) {
  data.datasets.forEach(({
    ...
    if (datasetType === 'area') {
      ...
      datasetPoints = (0,_utils_js__WEBPACK_IMPORTED_MODULE_1__.mergeArrays)([lowerBoundary, upperBoundary]);
    }
    if (datasetType === 'pie') {
...
/* harmony export */   isColorCloseToBackground: () => (/* binding */ isColorCloseToBackground),
/* harmony export */   isColorCloseToWhite: () => (/* binding */ isColorCloseToWhite)
/* harmony export */ });
function detectSkin() {
...
//# sourceMappingURL=src_lib_lovely-chart_LovelyChart_js.57a1d7b16231b14d8fdd.js.map