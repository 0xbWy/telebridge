...
const DEFAULT_APP_TITLE = `TeleBridge${APP_ENV !== 'production' ? ' Beta' : ''}`;
...
const CSP = `
  default-src 'self';
  connect-src 'self' wss://*.telebridge.online wss://*.web.telegram.org
    blob: http: https: ${APP_ENV === 'development' ? 'wss: ipc:' : ''};
